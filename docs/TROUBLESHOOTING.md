...
rm -rf ~/.cache/opencode/node_modules/opencode-antigravity-authlogin
rm -rf ~/.bun/install/cache/opencode-antigravity-authlogin*
opencode auth login
```
...
Remove-Item "$env:LOCALAPPDATA\opencode\Cache\node_modules\opencode-antigravity-authlogin" -Recurse -Force -ErrorAction SilentlyContinue
Remove-Item "$env:USERPROFILE\.bun\install\cache\opencode-antigravity-authlogin*" -Recurse -Force -ErrorAction SilentlyContinue
opencode auth login
```
...
```bash
cd ~/.cache/opencode
bun add opencode-antigravity-authlogin@latest
```
...
> If you intentionally run beta channel, use `bun add opencode-antigravity-authlogin@beta` instead.
...
1. **Update to latest beta:**
   ```json
   { "plugin": ["opencode-antigravity-authlogin@beta"] }
   ```
...
3. Update to latest beta: `"plugin": ["opencode-antigravity-authlogin@beta"]`
...
If you're seeing unusual rate limit behavior, please share in a [GitHub issue](https://github.com/D4ch1au/opencode-antigravity-authloginlogin/issues):
...
3. **Manual callback extraction** (advanced):
   ...
   - See [issue #119](https://github.com/D4ch1au/opencode-antigravity-authloginlogin/issues/119) for manual auth support
...
1. Ensure the plugin is installed: `"plugin": ["opencode-antigravity-authlogin@beta"]`
...
Open an issue on [GitHub](https://github.com/D4ch1au/opencode-antigravity-authloginlogin/issues).
